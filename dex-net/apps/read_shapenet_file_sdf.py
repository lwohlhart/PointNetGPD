...
def do_job_convert_obj_to_sdf(x):
    ...
    if not os.path.exists(str(file_list_all[x])+'/models/model_normalized.sdf'):
        generate_sdf(path_sdfgen, str(file_list_all[x])+'/models/model_normalized.obj', 100, 5)  # for google scanner
...
if __name__ == '__main__':
    ...
    #file_list_all = [f for f in file_list_all if not os.path.exists(f.replace('.obj', '.sdf'))]
    ...
    #     generate_obj_from_ply(i+'/models/model_normalized.obj')
    #     print("finish", i)
